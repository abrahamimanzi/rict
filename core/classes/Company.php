...
//		$user_ID = Session::get(Config::get('session/session_name'));
		...
		if(!$this->_db->insert('val_company', $fields)){
			throw new Exception("There was a problem creating your company.");
		}else{
            
            $companyTable = new Company();
            $companyTable->select("WHERE `vendor_ID`='{$fields['vendor_ID']}' ORDER BY `ID` DESC LIMIT 1");
            if($companyTable->count()){
                $company_data = $companyTable->first();
                $vendorTable = new Vendor();
                $vendorTable->update(array(
...
// USER DATA UPDATE
	public function update($fields = array(),$id = null){
		if(!$this->_db->update('val_company',$id,$fields)){
...
// FIND USER
	public function find($user = null,$limit = null){
		if($user){
			...
			if(is_numeric($user)){
				$field = 'ID';
				$data = $this->_db->get('val_company', array($field, '=', $user),$limit);
				if($data->count()){
	...
	public function select($sql = null){
		$data = $this->_db->query("SELECT* FROM `val_company` {$sql}");
		if($data->count()){
